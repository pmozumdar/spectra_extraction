...
class Spec1d(df.Data1d):
    ...
    def save(self, outfile, outformat='text', useresamp=False, verbose=True):
        ...
        if useresamp:
            ...
            vsize = len(wav)
            #var = None
            #need to decide how to deal with 'var' and 'sky' in case of resampling. But for now--
            varmod = interpolate.splrep(self['wav'], self['var'])
            var = interpolate.splev(wav, varmod)
            if self.sky:
                skymod = interpolate.splrep(self['wav'], self['sky'])
                sky = interpolate.splev(wav, skymod)
